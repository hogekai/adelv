...
function setupWithPlugin(options?: {
	...
	delivery.deliver(
		makeInput(ad, { purl: options?.purl, burl: options?.burl }),
	)
	...
	describe("burl + impression trackers on rendered", () => {
		...
		it("does not fire JAVASCRIPT method trackers", () => {
			...
			expect(calledUrls).not.toContain(
				"https://track.example.com/imp.js",
			)
		})
	})
